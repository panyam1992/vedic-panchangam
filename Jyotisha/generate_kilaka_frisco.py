...
def emit_kilaka_panchanga(panchaanga, output_stream, time_format="hh:mm:a", scripts=None):
    if scripts is None:
    ...
    # Kīlaka Samvatsara name (index 42 in 60-year cycle)
    samvatsara_name_dev = names.NAMES['SAMVATSARA_NAMES']['sa'][scripts[0]][42]

    # Title / Cover Page
    print(r'\mbox{}', file=output_stream)
    print(r'\renewcommand{\yearname}{2028--2029}', file=output_stream)
    print(r'\begin{center}', file=output_stream)
    print(r'{\sffamily \fontsize{60}{60}\selectfont  2028--2029\\[0.5cm]}', file=output_stream)
    print(r'\mbox{\fontsize{42}{42}\selectfont %s-नाम-संवत्सरः}\\[0.4cm]' % samvatsara_name_dev, file=output_stream)
    print(r'\mbox{\fontsize{22}{22}\selectfont (चैत्र-शुक्ल-प्रतिपद् [युगादि] तः फाल्गुन-कृष्ण-अमावास्या-पर्यन्तम्)}\\[0.3cm]', file=output_stream)
    print(r'\mbox{\fontsize{24}{24}\selectfont श्री शालिवाहन शक १९५० \quad कलियुगाब्द ५१२९–५१३०}\\[0.5cm]', file=output_stream)
...
def main():
    print("=" * 70)
    print("Generating Frisco TX Panchangam for Kīlaka Samvatsaram")
    print("Period: 2028-03-26 (Ugadi) to 2029-04-13 (Phalguna Amavasya)")
    print("=" * 70)
    ...
    print("\n[Step 1] Loading precomputed data for 2028 and 2029...")
    p2028 = annual.get_panchaanga_for_civil_year(city=city, year=2028, computation_system=comp_system, allow_precomputed=True)
    p2029 = annual.get_panchaanga_for_civil_year(city=city, year=2029, computation_system=comp_system, allow_precomputed=True)

    # Start: 2028-03-26 (Ugadi), End: 2029-04-13 (Phalguna Krishna Amavasya)
    # Include padding day 2028-03-25 (prior) and 2029-04-14 (posterior)
    combined_dict = {k: v for k, v in p2028.date_str_to_panchaanga.items() if k >= '2028-03-25'}
    combined_dict.update({k: v for k, v in p2029.date_str_to_panchaanga.items() if k <= '2029-04-14'})

    p2028.date_str_to_panchaanga = combined_dict
    p2028.start_date = Date(2028, 3, 26)
    p2028.end_date = Date(2029, 4, 13)

    days_in_panchanga = [dp for dp in p2028.daily_panchaangas_sorted() if p2028.start_date <= dp.date <= p2028.end_date]
    print(f"Total days in Kilaka Samvatsaram: {len(days_in_panchanga)}")
    ...
    eng_tex_name = 'Panchanga_Kilaka_Frisco_English.tex'
    ...
    print(f"Telugu PDF:     {os.path.join(telugu_dir, 'Panchanga_Kilaka_Frisco_Telugu.pdf')}")
    print(f"English PDF:    {os.path.join(english_dir, 'Panchanga_Kilaka_Frisco_English.pdf')}")
    print(f"Devanagari PDF: {os.path.join(deva_dir, 'Panchanga_Kilaka_Frisco.pdf')}")
    print("=" * 70)
