...
def _ordinal(n):
  ...
  if 11 <= n % 100 <= 13:
    return '%d\\textsuperscript{th}' % n
  return '%d\\textsuperscript{%s}' % (n, {1: 'st', 2: 'nd', 3: 'rd'}.get(n % 10, 'th'))
...
def _format_moon_time(moon_jd, jd, page_date, time_format):
  """Format a moonrise/moonset time, appending the date if it differs from the page date."""
  hours = 24 * (moon_jd - jd)
  time_str = time.Hour(hours).to_string(format=time_format)

  # Determine which calendar day the moon event actually falls on
  if hours >= 24:
    # Next day (or later)
    days_offset = int(hours // 24)
    actual_date = page_date.offset_date(days=days_offset)
    date_label = '{\\scriptsize on %s %s}' % (MONTH_NAMES[actual_date.month], _ordinal(actual_date.day))
    time_str += ' ' + date_label
  elif hours < 0:
    # Previous day (rare but possible)
    actual_date = page_date.offset_date(days=-1)
    date_label = '{\\scriptsize on %s %s}' % (MONTH_NAMES[actual_date.month], _ordinal(actual_date.day))
    time_str += ' ' + date_label
...
def stream_sun_moon_rise_data(daily_panchaanga, output_stream, time_format):
  ...
  page_date = daily_panchaanga.date

  # Format moonrise with date annotation if on a different day
  if daily_panchaanga.graha_rise_jd[Graha.MOON] > daily_panchaanga.jd_next_sunrise:
    moonrise = '---'
  else:
    moonrise = _format_moon_time(daily_panchaanga.graha_rise_jd[Graha.MOON], jd, page_date, time_format)

  # Format moonset with date annotation if on a different day
  if daily_panchaanga.graha_set_jd[Graha.MOON] > daily_panchaanga.jd_next_sunrise:
    moonset = '---'
  else:
    moonset = _format_moon_time(daily_panchaanga.graha_set_jd[Graha.MOON], jd, page_date, time_format)
